...
#include "config.h"
	...
	#define ENC28J60_PORT		PORTB
		...
		#if USE_AVRNETIO // Pollin AVR-NET-IO

			#define ENC28J60_PIN_SS		4
			#define ENC28J60_PIN_CS		4

        #else // Ulrich Radigs Board
	...
	#define delay(x) for(unsigned long a=0;a<x;a++){asm volatile ("nop" ::);}

/*  removed for 1284p due to compiler errors
